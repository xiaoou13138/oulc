...
public class HtmlParserTool {
	...
	public static Set<String> getMeta(String url){
		...
		try {
			...
			nodeFilters[0] = new NodeClassFilter(MetaTag.class);
			OrFilter linkFilter = new OrFilter(nodeFilters);
			// 得到所有经过过滤的标签
			NodeList list = parser.extractAllNodesThatMatch(linkFilter);
			for (int i = 0; i < list.size(); i++) {
				Node tag = list.elementAt(i);
				if (tag instanceof MetaTag){  // <a> 标签
					MetaTag meteTab = (MetaTag) tag;
					Object charsetObj = meteTab.getAttribute("charset");
					if(charsetObj != null){
